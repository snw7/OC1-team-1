...
```{r}
training_NN <- subset(df, Datum >= "2013-07-01" & Datum <= "2018-07-30")
validation_NN <- subset(df, Datum >= "2018-07-31" & Datum <= "2019-06-08")
test_NN <- subset(df, Datum >= "2019-06-09" & Datum <= "2019-07-30")

```

```{r}
# training_features <- as_tibble(model.matrix(Umsatz ~ as.factor(Warengruppe) + Temperatur + as.logical(KielerWoche) + as.factor(Monat) * Temperatur + as.factor(Monat) + as.factor(Wochentag) + as.factor(Bewoelkung) + as.factor(Windgeschwindigkeit) + Datum, training_NN))
training_features <- as_tibble(model.matrix(Umsatz ~ as.factor(Warengruppe) +  as.factor(Wochentag) + as.factor(Monat) + Datum + Bewoelkung + Temperatur + KielerWoche + Windgeschwindigkeit, training_NN))

# validation_features <- as_tibble(model.matrix(Umsatz ~ as.factor(Warengruppe) + Temperatur + as.logical(KielerWoche) + as.factor(Monat) * Temperatur + as.factor(Monat) + as.factor(Wochentag) + as.factor(Bewoelkung) + as.factor(Windgeschwindigkeit) + Datum, validation_NN))
  
validation_features <- as_tibble(model.matrix(Umsatz ~ as.factor(Warengruppe) +  as.factor(Wochentag) + as.factor(Monat) + Datum + Bewoelkung + Temperatur + KielerWoche + Windgeschwindigkeit, validation_NN))

# test_features <- as_tibble(model.matrix(Umsatz ~ as.factor(Warengruppe) + Temperatur + as.logical(KielerWoche) + as.factor(Monat) * Temperatur + as.factor(Monat) + as.factor(Wochentag) + as.factor(Bewoelkung) + as.factor(Windgeschwindigkeit) + Datum, test_NN))

test_features <- as_tibble(model.matrix(Umsatz ~ as.factor(Warengruppe) +  as.factor(Wochentag) + as.factor(Monat) + Datum + Bewoelkung + Temperatur + KielerWoche + Windgeschwindigkeit, test_NN))

training_label <- tibble(label=training_NN$Umsatz)

validation_label <- tibble(label=validation_NN$Umsatz)

test_label <- tibble(label=test_NN$Umsatz)
...
# test_features$`as.factor(Warengruppe)Saisonbrot` <- 0
...
history_1 = model_1.fit(r.training_features, r.training_label, epochs=800,
...
model_1.save("../ML_models/mod_nn_24.h5")


```
```{r}
# model_test <- load_model_hdf5("../ML_models/mod_nn_19.h5")
# 
# model_history <- model_test$history
...
# data_2 <- data.frame(val_loss = unlist(model_history$history$val_loss),
#                   loss = unlist(model_history$history$loss))
# 
...
# Need to save the predicted values in a .csv file.
...
# data_validation <- data.frame(prediction = validation_predictions/1000, actual = validation_label[[1]]/1000)
...
# predictions_temp <- read_csv("../data/prediction_template.csv")
# 
# predictions <- as.data.frame(test_predictions)
# 
# predictions$Datum <- predictions_temp$Datum
#   
# predictions <- left_join(predictions, predictions_temp, by = "Datum")
# 
# predictions$Umsatz <- predictions$V1
# 
