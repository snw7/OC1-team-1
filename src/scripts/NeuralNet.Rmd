...
features <- as_tibble(model.matrix(Umsatz ~ as.factor(Warengruppe) + as.factor(Wettercode) +  as.factor(Wochentag) + as.factor(Monat) + Datum + Bewoelkung + Temperatur + KielerWoche + Windgeschwindigkeit, df))
names(features)
...
model_2.compile(loss="mse", optimizer=Adam(learning_rate=0.01))
model_3.compile(loss="mse", optimizer=Adam(learning_rate=0.01))

# Schaetzung des Modells
history_1 = model_1.fit(r.training_features, r.training_label, epochs=200,
                    validation_data = (r.validation_features, r.validation_label), verbose=0)

# Ggf. Speichern des geschaetzten Modells
model_1.save("../ML_models/mod_nn_4.h5")
model_2.save("../ML_models/mod_nn_5.h5")
model_3.save("../ML_models/mod_nn_6.h5")
